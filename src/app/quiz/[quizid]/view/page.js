...
export default function Page({ params }) {
  ...
  useEffect(() =>{
    axios.get('/api/quiz?id='+params.quizid).then((res) => {
      ...
      setDataStatus(1)
        setQuizDetails({...res.data.quiz , createdBy: resp.data.data.name})
      }).catch((err) => {
      setDataStatus(1)
        setQuizDetails({...res.data.quiz , createdBy: 'Unnamed'})
      })
      }).catch((e) => {
        setDataStatus(-1)
        console.log(e)
    ...
    try {
      quizDetails.questions.forEach((question) => {
        ...
        if (userAnswers.length === 0) {
          ...
          const correct = correctAnswers.every((answer) => userAnswers.includes(answer));
          const wrong = userAnswers.some((answer) => !correctAnswers.includes(answer));
    ...
    <button>close</button>
      ...
      <div className="min-h-screen px-4 py-2 pb-12" data-theme={quizDetails.theme}>
        ...
        />
          ...
          <div className="flex flex-col justify-center items-start mt-4 gap-y-5">
              ...
              <div className="rounded-md px-3 py-4 shadow-md w-full shadow-neutral" key={question._id}>
                ...
                <div className="flex flex-col justify-center items-start gap-2 mt-3">
                  {question.options.map((option) => (
                    <label className="flex flex-row whitespace-break-spaces justify-start items-center gap-2" key={option.id}>
                      <input
