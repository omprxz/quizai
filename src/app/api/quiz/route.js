  ...
  async function generateQuiz(
    ...
    if (filesP) {
      ...
      for (const file of filesP) {
        if (file.size > maxFileSize) {
          return NextResponse.json(
            {
              message: `Max file size limit is ${
        ...
        const filePath = path.join(
          uploadsDir,
          `${path.basename(file.name, path.extname(file.name))}-${Math.random()
            .toString(36)
            .substr(2, 8)}${path.extname(file.name)}`
        );
      ...
      fileParts = await Promise.all(
        savedFiles.map((file) =>
          uploadToGemini(file.path, file.mimeType, apiKey)
        )
      );
    }
    ...
    try {
      ...
      if (error?.status == 429 || error?.status == 500) {
        generateQuiz(inputDataP, "gemini-2.0-flash", filesP);
...
*/
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          message: "Passing score must be between 0 and 100, or leave empty.",
          success: false,
        },
        { status: 400 }
      );
    ...
    console.log("DB Quiz Data:", JSON.stringify(quizData, null, 2));
    if (title) {
...
export async function GET(req) {
  ...
  try {
    ...
    let quizDetails;
    if (filter === "view") {
      quizDetails = await Quiz.findById(id).select(
        "-questions.correct_answers -questions.reason"
      );
    } else {
